...
def create_x():
    ...
    for coordinate in coordinates:
        ...
        APIHelper.PolyanetGenerator.generate_astral_object(
...
def create_crossmint_logo():
    ...
    for row in range(0, row_len):
        for col in range(0, col_len):
            attribute_astral_object = MatrixUtils.get_attribute_astral_object(
                matrix[row][col]
            )
            if not attribute_astral_object:
                continue
            print("Updating coordinate: [" + str(row) + ", " + str(col) + "]")
            astral_object_selector_class = ASTRAL_OBJECT_SELECTOR.get(
                attribute_astral_object[1]
            )
            input_data = {"row": row, "column": col}
            if astral_object_selector_class["attribute"] is not None:
                input_data[astral_object_selector_class["attribute"]] = (
                    attribute_astral_object[0]
                )
